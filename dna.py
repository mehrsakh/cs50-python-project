...
def main():
    ...
    with open(sys.argv[1], newline='') as csvfile:
        ...
        dnafile = {row['name']: [int(row[s]) for s in sequences] for row in reader}

    with open(sys.argv[2], 'r') as txtfile:
        dna_sequence = txtfile.read().strip()

    result = [longest_match(dna_sequence, subseq) for subseq in sequences]

    for name, counts in dnafile.items():
        if result == counts:
            print(name)
            return
